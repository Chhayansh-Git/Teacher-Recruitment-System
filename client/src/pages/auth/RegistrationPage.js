...
export const RegistrationPage = () => {
    ...
    const [formData, setFormData] = useState({
        email: '', contactNo: '', fullName: '', password: '', confirmPassword: '', type: 'teaching', position: '',
        name: '', affiliationNo: '', address: '', location: '', pincode: '', whatsappNumber: '', website: '',
        principalName: '', directorName: '', strength: '', schoolUpto: 'XII', board: 'CBSE', acceptedTerms: false,
    });
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            setSuccess(`${message} You will be redirected to the OTP page shortly.`);
            setTimeout(() => {
                navigate('/login');
            }, 3000);
        } catch (err) {
    ...
    const { gradient, title } = getRoleConfig();
                ...
                <div className="p-8">
                    ...
                    <form onSubmit={handleRegister} className="space-y-4">
                        ...
                        <div className="mt-6 text-center">
                            <p className="text-sm text-gray-600">Already have an account?{' '}<Link to="/login" className="font-medium text-blue-600 hover:underline">Sign In</Link></p>
                        </div>
                    </form>
                </div>
            </div>
...
const CandidateFormFields = ({ formData, handleInputChange, showPassword, setShowPassword }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <InputField icon={<BookUser />} name="position" value={formData.position} onChange={handleInputChange} placeholder="Position (e.g., Primary Teacher)" required />
    </div>
);
